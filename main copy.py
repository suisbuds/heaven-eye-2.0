...
class YoloPredictor(BasePredictor, QObject):
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None):
        ...
        # 将保存路径设置在当前根目录下的 runs/detect 文件夹中
        # project = root_dir / 'runs/detect' # BUG: 根目录路径是通过SEETINGS逻辑动态生成的
        project = (
            self.args.project or root_dir / "runs" / self.args.task
        ...
        try:
            ...
            while True:
                ...
                if self.continue_dtc:
                    ...
                    if (
                        count % 5 == 0 and count >= 5
                    ):  # Calculate the frame rate every 5 frames
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def change_val(self, x, flag):
        if flag == "iou_spinbox":
            ...
            self.show_status(
                "IOU Threshold: %s" % str(x / 100)
            )  # Intersection over Union，表示预测框和真实框的重叠程度，用来判断检测效果
            ...
            self.show_status(
                "Confidence Threshold: %s" % str(x / 100)
            )  # 置信度，表示模型对检测结果的置信程度
    ...
    def closeEvent(self, event):
        ...
        if self.yolo_thread.isRunning():
            ...
            MessageBox(
                self.close_button, title="Note", text="Exiting", time=3000, auto=True
            ).exec()
            sys.exit(0)
        else:
            sys.exit(0)
